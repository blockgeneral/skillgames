/**
 * Pure-JS SHA-256 for isomorphic use (Node + browser, synchronous).
...
export function sha256(message: string): string {
  ...
  for (let b = 0; b < blocks; b++) {
    ...
    for (let t = 0; t < 64; t++) {
      ...
      const t2 = (sigma0(a) + maj(a, b_, c)) >>> 0;
      h = g;
      g = f;
      f = e;
      e = (d + t1) >>> 0;
...
function pad(bytes: Uint8Array): Uint8Array {
  ...
  // Write bit length as 64-bit big-endian (we only use lower 32 bits for practical sizes)
